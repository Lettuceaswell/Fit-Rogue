# Fit-Rogue — Design Doc (v0.2.2)

A single-file, mobile-first, roguelike life-simulator. You play one life from age 19 to Retirement (67) in 2-year rounds, allocating a shared energy budget across work, training, and diet while managing an ever-rising stress floor. The game ends in one of two states: Crushed by stress, or retired well.
...
2. **Cards** Offer a draft of 3 upgrade cards; the player picks 1 to install (or resolves the slot conflict if it replaces a held card). Declining all 3 pays out a flat cash amount instead. Also offer Overtime slider which will yeild more cash per hour, but raise stress beyond expected next round. Also Offer PTO button then confirmation button - PTO will skip any money earned, but will put a dent in this round's stress.
...
| **Dollars** | Earned from job pay + overtime; spent on all card upgrades (job tier, spouse, gym) and diet/gym upkeep. Not itself a loss condition, but debt is allowed, and it will raise stress level. Cards are the primary money sink — they're priced to compete directly with PTO, so PTO isn't strictly correct whenever affordable. |
...
| **Muscle** | 6 stat levels: 1, 2, 3, 4, 5, 6. A higher muscle stat Helps you to burn excess energy more aggressively. You cannot gain a muscle level without gaining 1 or more Body fat levels at the same time, unless a held card overrides this rule. If one or more fat levels are gained, the muscle level will be boosted by one stat level.
...
Five named slots, each holding one card at a time: **Diet**, **Gym**, **Training Style**, **Partner**, **Job**. A held card is always replaceable when a new card is drafted for its slot.

Each slot owns exactly one knob, so future cards can't all collapse into "+training score": Diet sets energy IN (the fill ceiling and its conversion rules), Gym sets the Dollars cost of access to training, Training Style sets the energy-to-score conversion formula, Partner sets stress and/or the economy, and Job sets the Dollars ladder (a tier ramp, not a lateral choice).
